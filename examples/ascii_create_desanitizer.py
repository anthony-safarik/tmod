...
def create_python_script(script_name: str, script_contents: str, directory: str = "."):
    # Ensure the directory exists
    script_path = Path(directory) / script_name
    script_path.parent.mkdir(parents=True, exist_ok=True)

    # Write the Python code to the new file
    with script_path.open("w", encoding="utf-8") as f:
        f.write(script_contents)

    print(f"Script created: {script_path.resolve()}")

def create_desanitizer(folder_path):
    create_python_script("desanitizer.py",SCRIPT_CODE,folder_path)
...
create_python_script("generated_script.py", SCRIPT_CODE)
