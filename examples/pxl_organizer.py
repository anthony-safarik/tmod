...
def organize_pxl_files(source_dir, target_root):
    # Regex pattern to match filenames like PXL_20210606_220053516.jpg
    pattern = re.compile(r'^PXL_(\d{4})(\d{2})(\d{2})_.*\.(jpg|jpeg|png|mp4)$', re.IGNORECASE)
...
def main():
    parser = argparse.ArgumentParser(description="Organize PXL files into date-based folders.")
    ...
    organize_pxl_files(args.source, args.target)
