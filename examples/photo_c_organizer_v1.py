...
def find_files(directory):
    ...
    for root, _, files in os.walk(directory):
        for filename in files:
            ...
            if name.startswith("C") and len(name) == 5:
...
def main():
    parser = argparse.ArgumentParser(description="Find files that start with 'C' and are 8 characters in lenght and rename based on dated folder.")
    parser.add_argument("directory", help="Directory to search")
    parser.add_argument("target", help="Path to the target directory")
    args = parser.parse_args()

    matches = find_files(args.directory)
